...
public class EsStackingAlgo2 {
    ...
    public static short[] Run(int from, int to, ShotUtils utils, Point p[]){
        ...
        MyPoint vec3 = new MyPoint(((double)(p[2].x)-p[3].x)/(width),((double)(p[2].y)-p[3].y)/(width));
        MyPoint vec4 = new MyPoint(((double)(p[1].x)-p[3].x)/utils.height,((double)(p[1].y)-p[3].y)/utils.height);
        MyPoint vec13 = new MyPoint(vec1.x+vec3.x,vec1.y+vec3.y);
        vec13.mul(1);
        ...
        Clu.programSource = "#define cords(x,y)(y*"+utils.width*3+" + x*3)" +
                ...
                "float2 vec1 = (float2)("+vec1.x+","+vec1.y+");" +
                "float2 vec2 = (float2)("+vec2.x+","+vec2.y+");" +
                "float2 vec13 = (float2)("+vec13.x+","+vec13.y+");" +
                "float2 vec24 = (float2)("+vec24.x+","+vec24.y+");" +
                "int x2 = (int)((vec1.x*x+vec24.x*y)+(vec2.x*y+vec13.x*x));" +
                "int y2 = (int)((vec1.y*x+vec24.y*y)+(vec2.y*y+vec13.y*x));" +
                "if(x2>="+utils.width+"){x2 = "+(utils.width-1)+";}"+
