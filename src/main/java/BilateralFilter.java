public class BilateralFilter {
    ...
    private int[][] getPixels(int[] bitmap, int Width, int Height){
        int[][] output = new int[Width][Height];
        for(int h = 0; h<Height; h++)
            for(int w = 0; w<Width; w++) output[w][h] = bitmap[h*Width + w];
        return output;
    }
    public int[] Run(int[] input, int Width) {
        ...
        int[][] ra = getPixels(input, Width, Height);

        for (int h = 0; h < Height; h++) {
            for (int w = 0; w < Width; w++) {
                int res = apply(w, h, ra);


                result[h*Width+w] = res;
            }
        }
    ...
    private int apply(int w, int h, int[][] pixels) {

        double sum = 0;
        double totalWeight = 0;

        int mMax = w + kernelRadius;
        int nMax = h + kernelRadius;
        double weight;

        for (int m = w - kernelRadius; m < mMax; m++) {
            for (int n = h - kernelRadius; n < nMax; n++) {
                if (isInsideBoundaries(m, n, pixels)) {
                    weight = kernelD[w - kernelRadius+ m][h - kernelRadius + n] * similarity(pixels[m][n], pixels[w][h]);
                    totalWeight += weight;
